"""
Simple API Tests - Tests against running server
...
def test_load_data():
    ...
    print("\n[TEST] Load Data")
    response = make_request("/api/data/load")
...
def test_get_students():
    ...
    print("\n[TEST] Get Students")
    # Force data reload to ensure clean data
    make_request("/api/data/load")
    response = make_request("/api/students?limit=100")
...
def test_get_companies():
    ...
    print("\n[TEST] Get Companies")
    response = make_request("/api/companies?limit=100")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_summary_stats():
    ...
    print("\n[TEST] Summary Statistics")
    response = make_request("/api/stats/summary")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_department_stats():
    ...
    print("\n[TEST] Department Statistics")
    response = make_request("/api/stats/department")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_cgpa_stats():
    ...
    print("\n[TEST] CGPA Distribution")
    response = make_request("/api/stats/cgpa")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_company_stats():
    ...
    print("\n[TEST] Company Statistics")
    response = make_request("/api/stats/companies")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_domain_stats():
    ...
    print("\n[TEST] Domain Statistics")
    response = make_request("/api/stats/domain")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_departments_list():
    ...
    print("\n[TEST] Departments List")
    response = make_request("/api/departments")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_domains_list():
    """Test domains list"""
    print("\n[TEST] Domains List")
    response = make_request("/api/domains")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
...
def test_simulation_status():
    ...
    print("\n[TEST] Simulation Status")
    response = make_request("/api/simulation/status")
    assert response.status_code == 200, f"Failed with {response.status_code}"
    data = response.json()
